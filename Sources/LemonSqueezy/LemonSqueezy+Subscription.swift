...
extension LemonSqueezy {
    ...
    public func getSubscription(_ subscriptionId: Subscription.ID, queryItems: [URLQueryItem] = []) async throws -> LemonSqueezyAPIDataAndIncluded<Subscription, Subscription.Included> {
    ...
    public func getSubscriptions(pageNumber: Int = 1, pageSize: Int = 10, queryItems: [URLQueryItem] = []) async throws -> LemonSqueezyAPIDataIncludedAndMeta<[Subscription], Subscription.Included, Meta> {
        return try await call(route: .subscriptions, queryItems: queryItems, pageNumber: pageNumber, pageSize: pageSize)
    }
}
